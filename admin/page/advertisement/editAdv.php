...
<article>
     ...
     <table style="text-align:center;" border="1" class="table table-striped">
          ...
          <?php echo '
            <tr>
            <td>活动编号 </td>
            <td><input name="Id" value="'.$row["Id"].'" class="readonlyIp" readonly></td>
            <td>活动名称</td>
            <td><input name="activityName" value="'.$row["activityName"].'" ></td>
            </tr>

            <tr>
            <td>优惠券名称</td>
            <td><input name="tickName" value="'.$row["tickName"].'"></td>

            <td>活动推广图</td>
            <td><input  name="src" value="'.$row["src"].'" >
               </td>
               </tr>

               <tr>
              <td>是否开启</td>
            <td><input name="isActive" value="'?> <?php if($row["isActive"]==1){echo "是";}else{echo "否";}?><?php echo'" ></td>
            <td>活动名额</td>
            <td><input name="totalTick" value="'.$row["totalTick"].'"></td>
            ...
            <a href="advertisement.php?name='.$_REQUEST["adminname"].'" style="height:2rem;display:inline-block;width:5rem" class="btn btn-danger">返回</a>
